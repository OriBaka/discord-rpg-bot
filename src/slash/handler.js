...
const adminAutocomplete = require('./admin_autocomplete');
...
// Admin command names (để route sang admin handler)
const ADMIN_CMD_NAMES = new Set(adminDefs.map(d => d.data.name));

async function handleCommand(interaction, client) {
  // Route admin slash sang admin handler
  if (ADMIN_CMD_NAMES.has(interaction.commandName)) {
    return adminHandler.handle(interaction, client);
  }
...
async function handleAutocomplete(interaction) {
  // Admin autocomplete
  if (ADMIN_CMD_NAMES.has(interaction.commandName)) {
    return adminAutocomplete.handle(interaction);
  }
